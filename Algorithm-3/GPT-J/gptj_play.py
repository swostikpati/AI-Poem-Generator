...
from transformers import AutoTokenizer,GPTJForCasualLM
import numpy as np
import torch
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser("tool for running a pretrained gpt j")
    parser.add_argument("-m","--model",default="finetuned/checkpoint-2",type=str)
    ...
    if gpt6b is not None:
        ...
        model = GPTJForCasualLM.from_pretrained(model_to_load).half().to("cuda")
    else:
        tokenizer = AutoTokenizer.from_pretrained(model_to_load)
        model = GPTJForCasualLM.from_pretrained(model_to_load).to("cuda")
    # model = model.half()
